...
**Drain-table-mentions gate (S167) catches stale "data lives in PG" claims.** The reviewer-domain Postgres tables (`researchers`, `publications`, `researcher_keywords`, `reviewer_suggestions`, `grant_cycles`, `proposal_searches`) are drain-only post-W3-W6 cutover (2026-05-12); live source of truth is Dataverse (`wmkf_appresearcher`, `wmkf_potentialreviewer`, `wmkf_appreviewersuggestion`, `wmkf_appgrantcycle`). Three iterative S167 audits failed to converge on case-by-case fixes because the drift kept re-appearing in new docs; `npm run check:drain-table-mentions` is the mechanical fan-in. It flags any live doc/memory backticked mention of those six tables when no same-line context annotation is present. Annotation menu: cutover-history keywords (`drain`, `historical`, `RETIRED`, `post-W[3-6]`, `cutover`, `migrated`, `superseded`, `formerly`, `legacy`), Dataverse signal (mentioning a `wmkf_app*` entity on the same line implies migration context), or a structured marker `<!-- drain-table:ignore reason=<short> -->`. Files whose purpose IS describing the drained tables (atlas/postgres-*, migration plans, related memory entries) are allowlisted in the script. New PG-table references without an annotation are a signal that the ground-truth claim has changed — confirm with code-level evidence before broadening the allowlist or removing the gate.
