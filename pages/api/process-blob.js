  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const summary = await generateSummary(text, fileInfo.filename, apiKey);
        results[fileInfo.filename] = summary;

      } catch (fileError) {
        console.error(`Error processing ${fileInfo.filename}:`, fileError);
        results[fileInfo.filename] = createErrorResult(fileInfo.filename, fileError.message);
      }
    }
...
// Import utility functions from shared utils
