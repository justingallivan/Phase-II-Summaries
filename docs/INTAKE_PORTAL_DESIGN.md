...
> **Update 2026-05-18 (S163) — precondition status post Connor's S162 ruling.** Connor's deactivate-not-delete ruling **dissolves P2 and the Delete portion of P1** (no Delete trigger exists anymore — defunct children are deactivated via a `statecode` Update that carries the parent lookup; recompute is over active children only). **P3 landed S150.** P1's residual ("status filter binds on the **Update** trigger") is the docs' acknowledged clean case and is exercised post-deploy via the P4 proxy path — it does not gate creating the additive schema. **Consequence: no open Item-6 precondition gates the slice-0 schema deploy.** Full reasoning + the corrected Option B drain lifecycle (deactivate, not "delete old / insert new") in `INTAKE_PORTAL_ITEM_6_DISCUSSION.md` § 0 "Update 2026-05-18 (S163)."
