...
|---|---|---|---|---|---|
| Decision date | `akoya_decisiondate` | DateTime | 100 | 31 | **NOT a decided flag — it is an approval/decision-date *stamp* (terminal-declined/ineligible/Concept-Done rows have none). Decided-state now keyed off `akoya_requeststatus`, see Decided-state predicate below.** |
...
- **The predicate is a value→class map, not a single value:** `akoya_requeststatus ∈ {Phase I/II Pending, Concept Pending, Pending}` ⇒ in-flight (probe-clean: 0% leakage); `∈ {*Declined, *Ineligible, Denied, Closed, *Done, Approved, Rescinded}` ⇒ decided-terminal. The pending side is evidence-clean; the **ambiguous middle — `Active` (94% carry a date ⇒ likely *funded-in-progress*, not an in-flight application), `Withdrawn` (n=7, 0% date ⇒ behaves undecided), `Proposal Not Invited` — needs Connor sign-off** before it is authoritative. (`Withdrawn`/`Rescinded` are listed *only* here, not in the decided-terminal set, to avoid a double-assignment.)
...
**Residuals (updated 2026-05-16 — the Bucket-B-structural Connor item was resolved by probe, not Connor):**
- **(i) AkoyaGo / user (not Connor):** confirm the export-column *set* against the AkoyaGo trusted-view operational filters (gated step 2 / artifact 1 excavation).
- **(ii) Connor — one small sign-off:** the `akoya_requeststatus` decided-state class-map ambiguous middle (`Active`/`Withdrawn`/`Proposal Not Invited`). Folds into the existing Connor type-taxonomy session; not a new meeting. (The earlier "Connor must explain the B-structural drop" residual is **removed** — `scripts/probe-akoya-request-by-type.js` showed it is a migration-backfill artifact + Concept-type mix, no Connor needed.)
- **(iii) ~~Solo follow-up probe~~ — DONE (2026-05-16, `scripts/probe-akoya-native-request-amount.js`):** the native `Request`-type 32% residual is invited/discretionary awarded giving (577/582 `Approved`, 98% paid), requested-amount N/A by design. Puzzle 1 fully closed; no residual.
...
- **Track B — design converged; still blocked on the residual evidence tasks** before a build plan (semantic-layer artifacts must be evidence-derived, not invented — probe-before-plan). **Progress (2026-05-16):** artifact 2's era-boundary sub-part fully resolved by probe (cutover = 2023-12-03); **artifact 3 DELIVERED** (Field-Availability Boundary table + 5-bucket classification + disclosure-layer spec — see its section). Remaining blockers: **artifact 1** (plain-English field dictionary — needs the AkoyaGo trusted-view operational-filter excavation) + **artifact 2's *type* taxonomy portion** (Connor: recent-era authoritative definitions + remap history). Artifact 3 residuals (see the Artifact 3 "Residuals" block for the authoritative list): **(i)** AkoyaGo export-column-set confirmation (AkoyaGo/user, folds into artifact-1 excavation); **(ii)** one small Connor sign-off — the `akoya_requeststatus` class-map ambiguous middle — folded into the existing Connor type-taxonomy session; **(iii)** an optional solo follow-up probe on the native-`Request` 68% residual. The earlier "Connor must explain the B-structural drop" item was **resolved by probe** (migration backfill artifact + Concept-type mix), not Connor — so Connor's net Power Tools load is now just the (ii) sign-off.
