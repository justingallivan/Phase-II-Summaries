...
- **Era boundary quantified:** `createdon` is **2023-dominated (22,573 of ~25,561)** — a bulk migration import collapsed true historical dates onto ~2023. So `createdon` is **not** a usable Akoya-native-vs-legacy signal; the cutover is ~2023 (exact date → Connor / AkoyaGo, gated step 3).
- **`overriddencreatedon` DISCONFIRMED as an era marker (2026-05-16 corrected probe — `scripts/probe-akoya-overriddencreatedon.js`, FetchXML aggregates, never `$count`).** The step-1 residual is now closed conclusively, and the answer is negative: `overriddencreatedon` is **null on 100% of rows (0 / 25,561)** — the AkoyaGO bulk import never populated it. The prior "inconclusive" was a `$count`-cap artifact (the original probe counted the marker with the same `$count` it had just proved caps at 5,000). Boundary cross-tab confirms all 22,573 `createdon`-2023 rows are marker-null and no year carries the marker. **Design consequence:** there is **no system-field origin marker**; the *only* native-vs-migrated signal is the `createdon` 2023 collapse, and boundary *precision* now hard-depends on gated steps 2 (AkoyaGo cutover date) + 3 (Connor) — the solo Dataverse path is exhausted. The Track B semantic layer must treat "era" as derived from an externally-supplied cutover date applied against `createdon`, not from any per-row origin field.
...
1. **Claude → Dataverse probe — DONE (discriminator/volumes 2026-05-15 `scripts/probe-akoya-request-discriminators.js`; era residual closed 2026-05-16 `scripts/probe-akoya-overriddencreatedon.js`).** Composite discriminator + per-type volumes + ~25,561 true count + `$count`-caps-at-5000 + ~2023 migration spike captured (Track B "Evidence" + Atlas). The `overriddencreatedon` residual is **closed conclusively and negatively**: marker is null on 100% of rows, so no system-field origin marker exists — era detection must use an externally-supplied cutover date against `createdon`. **No solo Dataverse residual remains on step 1;** boundary *precision* is now fully a function of steps 2 + 3.
