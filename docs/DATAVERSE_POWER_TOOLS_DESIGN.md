...
**Residuals (two non-solo, both Connor-bound):** (i) confirm the export-column *set* against the AkoyaGo trusted-view operational filters (gated step 2 / artifact 1 excavation); (ii) Connor must explain the Bucket-B-structural migrated-100%/native-~46% drop (mapping vs intake-process) **and** sign off the `akoya_requeststatus` decided-state class map for the ambiguous middle (`Active`/`Withdrawn`/`Proposal Not Invited`) — this folds into the existing Connor type-taxonomy session (artifact 2), not a new task; the Pending-family side is already probe-clean. The earlier "status-stratify the migrated cohort" residual is **withdrawn** — migrated is degenerately ~100% decided, so a decided/not-decided split cannot exist there; that asymmetry is the lifecycle finding. This table is the evidence-derived candidate, not the final authoritative export contract.
