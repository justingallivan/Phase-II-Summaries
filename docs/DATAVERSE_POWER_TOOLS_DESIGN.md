...
**Bucket B-structural — LARGELY EXPLAINED (2026-05-16, `scripts/probe-akoya-request-by-type.js`). The "migrated 100% vs native ~46%" framing was invalid; superseded as follows:**

- **🔴 Migrated `akoya_request`/`akoya_expenses` 100% is a MIGRATION BACKFILL ARTIFACT, not captured data — Track B export hazard.** The Blackbaud import populated both fields on **100% of *every* `wmkf_request_type`**, including `Office Visit` (2,823) and `Phone Call` (913) — interaction logs that categorically have no budget. A field that is 100% on rows that cannot have a value is import-fill (0 or copied), not real. **Track B must NOT export migrated `akoya_request`/`akoya_expenses` as a real amount** — treat as `UNKNOWN — migration backfill` for migrated rows, or exclude.
- **The native gap is mostly request-type mix (expected, not a defect).** Native `Concept` (1,045 rows, 35% of native) is **11%** populated — Concepts are feedback-only, no budget (user-confirmed). That legitimately accounts for most of the native sparsity.
- **Residual real puzzle (narrowed):** native **`Request`-type** rows are **68%** populated (`akoya_request` 1,224/1,806), not ~100%. ~32% of genuine Akoya-era funding requests carry no requested amount. Hypotheses (untested): in-flight Requests not yet budgeted; amount relocated to `akoya_loirequestedamount` or the `wmkf_proposalbudgetline` child for portal/GOapply-origin rows. **This** is the only part still open — much smaller than the original framing, and no longer "needs Connor remap history" as a blanket.

| Export column | Field | Type | mig% (artifact) | nat% all | nat% Request-type only |
|---|---|---|---|---|---|
| Requested amount | `akoya_request` | Money | 100 (backfill) | 48 | **68** |
| Total project budget | `akoya_expenses` | Money | 100 (backfill) | 45 | **67** |
...
3. **Bucket-B-structural (`akoya_request`/`akoya_expenses`):** (a) **migrated rows ⇒ `UNKNOWN — migration backfill (not real)`** — never export the backfilled 100% as an amount; (b) native `Concept`/interaction-type ⇒ `N/A — feedback request, no budget`; (c) native `Request`-type null ⇒ `UNKNOWN — not captured` (the ~32% residual). Do NOT label any of these "not yet decided."
...
- **(iii) Solo follow-up probe (no one blocked):** quantify where the missing requested amount goes for the ~32% of native `Request`-type rows (candidates: `akoya_loirequestedamount`, `wmkf_proposalbudgetline` child). Optional refinement, not a build blocker.
...
- **Puzzle 1 (B-structural) — largely resolved by `scripts/probe-akoya-request-by-type.js` (user hypothesis test, S157).** User proposed: Concept requests are feedback-only (no budget) and inflate the native gap. Confirmed: native `Concept` `akoya_request` = 11% (legitimately no budget). Bigger finding: migrated 100% is a **migration backfill artifact** — 100% even on `Office Visit`/`Phone Call` rows that cannot have a budget — so migrated `akoya_request`/`akoya_expenses` must never be exported as a real amount (new Track B hazard). Residual narrowed to native `Request`-type at 68% (not ~100%); the "needs Connor remap history" residual is removed (resolved by probe), leaving only an optional solo follow-up on where the missing amount relocates.
