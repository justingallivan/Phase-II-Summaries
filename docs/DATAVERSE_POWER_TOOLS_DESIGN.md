...
3. **The native `Request`-type residual (32%, n=582) is invited/discretionary giving — N/A by design, not missing.** Of native `Request`-type rows with no ask in *either* `akoya_request` or `akoya_loirequestedamount` (LOI is a strict subset of `akoya_request` — 0 LOI-only rows, so it's not a phase-split): **577/582 are `Approved`, 93% carry `akoya_grant`, 98% are paid (`akoya_paid`>0)**. These are awarded, paid grants with no applicant-stated request — the foundation's invited/discretionary/staff-directed giving, where "requested amount" is not a meaningful concept. The awarded amount is the real figure.
