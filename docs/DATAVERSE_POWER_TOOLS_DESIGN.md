...
- **But `akoya_decisiondate` ≠ "decided".** The 4 pending values cover only 474 of the 2,057 native no-decision-date rows; the other ~1,583 are *terminal-decided* statuses (`Phase I Declined` 640, `Concept Done` 821, `Closed`, `Ineligible`, `Denied`) that carry **no decision date**. So `akoya_decisiondate` is an **approval/decision-date stamp** (present mainly for funded + some declined), not a "has been decided" flag — the earlier B-lifecycle "native decisiondate sparsity = not yet decided" reasoning was too coarse and is sharpened here.
- **The predicate is a value→class map, not a single value:** `akoya_requeststatus ∈ {Phase I/II Pending, Concept Pending, Pending}` ⇒ in-flight; `∈ {*Declined, *Ineligible, Denied, Closed, *Done, Approved, Withdrawn, Rescinded}` ⇒ decided-terminal. The pending side is evidence-clean; the **terminal/ambiguous middle (`Active`, `Withdrawn`, `Proposal Not Invited`) needs Connor sign-off** before it is authoritative.
