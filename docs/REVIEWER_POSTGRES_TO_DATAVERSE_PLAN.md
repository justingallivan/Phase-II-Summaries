...
1. **Decisions locked** (no longer dependency-order items): `researchers.js` retires; `extract-summary.js` retires entirely; cleanup-cron predicate; 14-day grace; junction approach; **W3 cutover method = Option B with loud-fail guard (locked S147)**.
...
15. **Cleanup cron real-mode.** Earliest 14 days post-cutover. **`scripts/restore-from-cleanup-backup.js` must exist before this step.**
