...
| `scripts/restore-from-cleanup-backup.js` | spec'd | Reverse the one-shot cleanup pre-delete backup blob. **Must exist before the post-pilot cleanup script runs in real mode** (per `Rollback strategy §3`); not "post-cutover hygiene." |
