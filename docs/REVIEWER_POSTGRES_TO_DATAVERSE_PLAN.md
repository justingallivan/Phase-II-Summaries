...
| `wmkf_appgrantcycle` entity | **DEPLOYED, DATAVERSE-PRIMARY** (10 custom attrs live, 10 rows post-W3 cutover 2026-05-12) | The entity is the live source of truth for cycle data. Three fields originally flagged as missing (`wmkf_ShortCode`, `wmkf_ProgramName`, `wmkf_CustomFields`) may still need a schema patch — re-verify via `scripts/dynamics-schema-diff.js wmkf_appgrantcycle` before any destructive Postgres action. See `docs/atlas/dataverse-wmkf-apppublication-and-appgrantcycle.md`. |
