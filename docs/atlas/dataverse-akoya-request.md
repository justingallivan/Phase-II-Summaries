...
**Era:** `createdon` by year — **2023: 22,573** · 2024: 1,167 · 2025: 1,376 · 2026: 445. ~88% stamped 2023 ⇒ a **bulk migration/import event ~2023** collapsed true historical creation dates. `createdon` is migration-dominated and is **NOT** a reliable Akoya-native-vs-legacy signal; the cutover is ~2023 (confirm exact date with Connor / AkoyaGo). `overriddencreatedon` — **DISCONFIRMED as an era marker (2026-05-16, `scripts/probe-akoya-overriddencreatedon.js`, FetchXML aggregates not `$count`):** it is **null on 100% of rows (0 / 25,561)**. The AkoyaGO bulk import did not populate it, so no system field preserves the true historical date. Boundary cross-tab: all 22,573 `createdon`-2023 rows are `overriddencreatedon`-null; 0 migrated-marker rows in any year. **Consequence:** the only system-field era signal is the `createdon` 2023 collapse; native-vs-migrated boundary precision now hard-depends on the AkoyaGO excavation + Connor's cutover date (no solo Dataverse path remains). The earlier "inconclusive" framing (groupby all-null + `$count` filter) was the `$count`-cap artifact, now resolved.
