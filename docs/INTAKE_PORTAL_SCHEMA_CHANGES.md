...
- **Item 6 — drain-vs-PA aggregate write conflict.** Decision locked 2026-05-14 as A+B hybrid. Three pre-deploy preconditions gate slice 0: maker-portal Tests 1+2 (Connor — pending) + rule-exception edit (landed S150 — see `INTAKE_PORTAL_DESIGN.md` § "Power Automate boundary" → "Exception — intake portal aggregate fields on `akoya_request`"). Precondition #4 (post-deploy real-schema verification) gates PA flow go-live, not slice 0 deploy.
...
- **Live-probe verification of `wmkf_apprequestperson.wmkf_role` before deploy.** Atlas + checked schema spec both list only PI=`100000000` / Co-PI=`100000001` as deployed values, but the live Dataverse option-set metadata has not been probed since S139. Pre-deploy step: run `node scripts/dynamics-schema-diff.js` (or equivalent EntityDefinitions probe) and confirm no existing values occupy `100000002`–`100000004`. Codex S150 flagged this as the only unverifiable claim in the slice 0 prep.
