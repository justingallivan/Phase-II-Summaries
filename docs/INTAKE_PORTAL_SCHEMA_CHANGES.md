...
- **Item 6 — drain-vs-PA aggregate write conflict.** Decision locked 2026-05-14 as A+B hybrid; rule-exception edit landed S150 (`INTAKE_PORTAL_DESIGN.md` § "Power Automate boundary" → "Exception — intake portal aggregate fields on `akoya_request`"). **Updated 2026-05-18 (S163): Item 6 no longer gates the slice-0 *schema* deploy.** Connor's S162 deactivate-not-delete ruling dissolved P2 (Delete-trigger parent-ID — no Delete trigger exists) and P1's Delete portion; P3 landed S150; P1's residual (status filter binds on the **Update** trigger, the docs' clean case) + P4 are post-deploy / PA-flow-live only. The earlier "Tests 1+2 (Connor — pending) gate slice 0" framing is **superseded** — see `INTAKE_PORTAL_ITEM_6_DISCUSSION.md` § 0 "Update 2026-05-18 (S163)" for the full reasoning + the corrected Option B drain lifecycle (deactivate, not delete-and-reinsert).
