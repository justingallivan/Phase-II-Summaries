 ...
 * EMERGENCY_AUTH_BYPASS=true is explicitly set. This prevents a misconfigured
 * deploy (missing AUTH_REQUIRED, missing Azure credentials) from silently
 * disabling auth on app-gated routes. Downstream session checks will then
 * 401 cleanly instead of bypassing.
 *
 * Non-production (dev / test): keeps the existing behavior where auth is
 * disabled if AUTH_REQUIRED!='true' or Azure credentials are missing — local
 * dev runs without Azure AD wired up.
 */
export function isAuthRequired() {
  ...
  if (isProduction && !emergencyBypass) {
    if (!authRequired) {
      console.error('[auth] AUTH_REQUIRED!=true in production — enforcing auth (set EMERGENCY_AUTH_BYPASS=true to override)');
    }
    if (!hasCredentials) {
      console.error('[auth] Azure AD credentials missing in production — enforcing auth (downstream calls will 401 until restored)');
    }
    return true;
  }

  if (isProduction && emergencyBypass) {
    console.warn('[auth] EMERGENCY_AUTH_BYPASS=true in production — authentication is disabled');
  }
...
export async function requireAppAccess(req, res, ...appKeys) {
  ...
  if (!cached || Date.now() - cached.loadedAt > APP_ACCESS_TTL_MS) {
    // Parallel fetch: app grants (backend-dispatched) + superuser check + active status
    const [appKeys, rolesResult, profileResult] = await Promise.all([
