...
class DatabaseService {
  ...
  // ============================================
  // RESEARCHER OPERATIONS — REMOVED in W5 (2026-05-12, commit pending)
  // ============================================
  //
  // The researcher, publication, keyword, and suggestion methods were gutted
  // after the three live callers (`discovery-service`, `deduplication-service`,
  // `contact-enrichment-service`) migrated to Dataverse adapters in commit
  // c0c5b5b. The legacy Postgres-researchers cache + writer is no longer
  // reachable from any code path.
  //
  // For replacements, see:
  //   - `lib/utils/name-normalization.js`             (pure name normalization)
  //   - `lib/dataverse/adapters/potential-reviewer`   (person upsert by email)
  //   - `lib/dataverse/adapters/researcher`           (bibliometric sidecar)
  //   - `lib/dataverse/adapters/reviewer-suggestion`  (per-(person, request) ledger)
  //
  // The Postgres tables (researchers, researcher_keywords, publications,
  // reviewer_suggestions) remain readable until post-pilot table drops (W6
  // sets the tables read-only and prepares the drop; the actual DROP TABLE
  // happens 14+ days post-cutover, per plan §"Revised pilot timing"). This
  // gut only removes write/read paths exposed via this service class.
