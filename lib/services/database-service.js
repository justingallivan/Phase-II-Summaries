...
// Lazy-loaded to avoid unnecessary Dataverse client construction when the
// flag is off. Each method in the USER PREFERENCE section dispatches to
// this when WAVE1_BACKEND_PREFS === 'dataverse'. Default is postgres.
let _dataversePrefs;
function getDataversePrefsService() {
  if (_dataversePrefs) return _dataversePrefs;
  _dataversePrefs = require('./dataverse-prefs-service');
  return _dataversePrefs;
}
function useDataversePrefs() {
  return process.env.WAVE1_BACKEND_PREFS === 'dataverse';
}
