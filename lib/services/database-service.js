 ...
 *   - User preferences dispatcher — `getUserPreferences`,
 *     `setUserPreference(s)`, `deleteUserPreference`, `hasPreference`,
 *     `getDecryptedApiKey`. The Postgres branch is dead code retained
 *     pending a separate cleanup pass (`useDataversePrefs()` returns
 *     true outside of an explicit `WAVE1_BACKEND_PREFS=postgres`
 *     override); production goes through the Dataverse adapter chain.
 *
