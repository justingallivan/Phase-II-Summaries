...
export class LiteratureSearchService {
  ...
  static async searchAll(claimData) {
    ...
    const piNames = claimData.piNames || [];
   ...
   */
  static async _searchPIPubs(piNames) {
    const apiKey = process.env.SERP_API_KEY;
    if (!apiKey || piNames.length === 0) return [];

    const results = [];
    for (const piName of piNames.slice(0, 3)) {
      try {
        const params = new URLSearchParams({
          engine: 'google_scholar',
          q: `author:"${piName}"`,
