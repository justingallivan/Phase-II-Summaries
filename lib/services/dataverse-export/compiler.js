...
const AXES = {
  ...
  // akoya_requeststatus is a String field (100% both cohorts) — a filter
  // literal is compiled literally; living-taxonomy currency is a PREVIEW
  // concern, never a compiler 422 (§2.1 point 4).
  status: { field: 'akoya_requeststatus', kind: 'identity' },
...
function compileFilter(f) {
  const axisDef = AXES[f.axis];
  const field = f.axis === 'amount' ? AMOUNT_WHICH[f.which] : axisDef.field;
...
function compile(querySpec, opts = {}) {
  ...
  // correctness invariant; /$count silently caps at 5,000). No link-entity /
  // no all-attributes in the count fetch (aggregate fetch must be minimal).
  const countFetchXml =
