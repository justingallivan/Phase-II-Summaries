...
// Suffix fallbacks (design doc §305: `∈ {*Declined, *Ineligible, …}`) for
// future Phase-prefixed variants not yet enumerated above. Applied ONLY when
// the exact-value map misses — never overrides an explicit entry. Still a
// classification, never a silent coerce: the row keeps its raw value and the
// engine flags suffix-matched rows in the methods sheet.
const STATUS_SUFFIX_RULES = [
  { suffix: 'Pending', class: STATUS_CLASS.IN_FLIGHT },
  { suffix: 'Declined', class: STATUS_CLASS.DECIDED_NO_AWARD },
  { suffix: 'Ineligible', class: STATUS_CLASS.DECIDED_NO_AWARD },
];
 ...
 * @returns {{class, provenance, matchedBy:'exact'|'suffix'}} | null (null ⇒
...
function classifyStatus(raw) {
  ...
  for (const rule of STATUS_SUFFIX_RULES) {
    if (v.endsWith(rule.suffix)) {
      return { class: rule.class, provenance: 'suffix-rule', matchedBy: 'suffix' };
    }
  }
  return null;
}
...
// Sharpest SINGLE reviewer-exclusion predicate (design doc §"(ii)"): the
// honorarium funding category. Offered as the precise one-clause exclusion.
