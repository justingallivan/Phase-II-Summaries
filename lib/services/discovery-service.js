  ...
  static async verifyClaudeSuggestions(suggestions, onProgress) {
    ...
    for (let i = 0; i < suggestions.length; i++) {
      ...
      if (finalArticles.length >= this.MIN_PUBLICATIONS) {
        ...
        // Verification requirement: Must have at least MIN_PUBLICATIONS results from
        // disambiguated search (name + expertise keywords). This ensures we're finding
        // the right person with relevant expertise, not just anyone with that name.
        //
        // This filters out weak matches like "Benjamin Kerr" (only 1 disambiguated result)
        // while keeping legitimate experts like "Curtis Suttle" who have many publications
        // even if the expertise keyword matching is imperfect.
        if (dedupedDisambiguated.length < this.MIN_PUBLICATIONS) {
          const reason = `Only ${dedupedDisambiguated.length} publications found with expertise keywords (need ${this.MIN_PUBLICATIONS})`;
          ...
          if (expertiseMatch < 0.35) {
            console.log(`Low expertise match for ${suggestion.name}: ${Math.round(expertiseMatch * 100)}% - accepting (has ${dedupedDisambiguated.length} disambiguated publications)`);
          }
