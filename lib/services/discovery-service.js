...
class DiscoveryService {
   ...
   *
   * @param {string} verifiedAffiliation - Affiliation found via PubMed
   * @param {string} suggestedInstitution - Institution Claude suggested
  ...
  static checkInstitutionMismatch(verifiedAffiliation, suggestedInstitution) {
    ...
    const normalizeInstitution = (inst) => {
      return inst
        .toLowerCase()
        .replace(/^(the\s+)/i, '')
        .replace(/\s+(university|institute|college|school|center|centre)/gi, ' university')
        .replace(/\s+/g, ' ')
        .trim();
    };

    const verified = normalizeInstitution(verifiedAffiliation);
    const suggested = normalizeInstitution(suggestedInstitution);

    // Check for common abbreviations and variations
    const institutionAliases = {
      'mit': ['massachusetts institute of technology', 'mit'],
      'caltech': ['california institute of technology', 'caltech'],
      'uc berkeley': ['university of california berkeley', 'uc berkeley', 'ucb'],
    ...
    for (const aliases of Object.values(institutionAliases)) {
      const verifiedMatches = aliases.some(a => verified.includes(a));
      const suggestedMatches = aliases.some(a => suggested.includes(a));
      if (verifiedMatches && suggestedMatches) {
        return false; // Same institution via alias
      }
    }

    // Extract key institution name (first significant words)
    const extractKey = (text) => {
      // Remove department, state, country info
      const parts = text.split(',')[0];
      // Look for "university of X" or "X university" patterns
      const uniMatch = parts.match(/(university of \w+|\w+ university|\w+ institute)/i);
      return uniMatch ? uniMatch[1] : parts.substring(0, 30);
    };

    const verifiedKey = extractKey(verified);
    const suggestedKey = extractKey(suggested);

    // Check if keys are similar (one contains the other or significant overlap)
    if (verifiedKey.includes(suggestedKey) || suggestedKey.includes(verifiedKey)) {
      return false; // Match
    }

    // Check for word overlap (at least 2 significant words in common)
    const verifiedWords = verifiedKey.split(' ').filter(w => w.length > 3);
    const suggestedWords = suggestedKey.split(' ').filter(w => w.length > 3);
    const commonWords = verifiedWords.filter(w => suggestedWords.includes(w));

    if (commonWords.length >= 2) {
      return false; // Enough overlap to consider a match
    }
