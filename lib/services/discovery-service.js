...
class DiscoveryService {
  ...
  static async verifyClaudeSuggestions(suggestions, onProgress) {
    ...
    for (let i = 0; i < suggestions.length; i++) {
      ...
      if (finalArticles.length >= this.MIN_PUBLICATIONS) {
        ...
        // NOTE: Previously had aggressive rejection logic here based on expertise keyword matching.
        // This was removed because it incorrectly rejected legitimate experts like Curtis Suttle,
        // Forest Rohwer, Jennifer Martiny, Joshua Weitz, and Benjamin Kerr.
        //
        // The problem: Claude's expertiseAreas use different terminology than paper titles,
        // and PubMed results often lack abstracts, so keyword matching fails even for correct matches.
        //
        // Now we verify based on finding sufficient publications (MIN_PUBLICATIONS threshold)
        // and show the confidence score in the UI as a warning rather than rejecting outright.

        // Log low confidence for debugging, but don't reject
        if (expertiseMatch < 0.35) {
          console.log(`Low expertise match for ${suggestion.name}: ${Math.round(expertiseMatch * 100)}% - but still accepting (has ${finalArticles.length} publications)`);
        }
      ...
      for (const article of articles) {
        if (article.authors && article.authors.length > 0) {
          ...
          if (seniorAuthor) {
            candidates.push({
              name: typeof seniorAuthor === 'string' ? seniorAuthor : seniorAuthor.name,
              affiliation: typeof seniorAuthor === 'object' ? seniorAuthor.affiliation : undefined,
              publications: [{
                title: article.title,
                year: article.year,
                doi: article.doi,
   ...
   *
   * Uses stricter matching to avoid false positives from generic words:
   * - Requires matching multiple keywords from the same expertise area
   * - Or matching the full phrase
   */
  static calculateExpertiseMatch(articles, expertiseAreas) {
    ...
    // For each expertise area, create matching criteria
    // An article matches an expertise area if:
    // 1. It contains the full phrase (best match), OR
    // 2. It contains at least 2 significant words from the phrase
    const expertiseCriteria = expertiseAreas.map(area => {
      const lowerArea = area.toLowerCase();
      const words = lowerArea.split(/[\s,]+/).filter(w => w.length > 3);
      return {
        fullPhrase: lowerArea,
        words: words,
        // Require at least 2 words to match, or all if fewer than 2
        minWordsToMatch: Math.min(2, words.length)
      };
    });

    // Count articles that match at least one expertise area well
    let strongMatches = 0;
    for (const article of articles) {
      const searchText = `${article.title || ''} ${article.abstract || ''}`.toLowerCase();

      for (const criteria of expertiseCriteria) {
        // Check for full phrase match (strongest signal)
        if (searchText.includes(criteria.fullPhrase)) {
          strongMatches++;
          break;
        }

        // Check for multiple word matches from the same expertise area
        const wordMatches = criteria.words.filter(word => searchText.includes(word));
        if (wordMatches.length >= criteria.minWordsToMatch) {
          strongMatches++;
          break;
        }
      }
    ...
    // Calculate confidence as percentage of strongly matching articles
    const confidence = strongMatches / articles.length;
