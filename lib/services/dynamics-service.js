...
// Module-level restrictions state (set per-request by chat handler)
// Initialized to null (fail-closed): queries will throw until setRestrictions() is called.
...
export class DynamicsService {
  ...
  /**
   * Set active restrictions for the current request. Use this for flows that
   * legitimately need field/table masking (Dynamics Explorer chat).
   *
   * For trusted internal callers that intentionally bypass all restrictions
   * (API endpoints, scripts), call `bypassRestrictions(requestId)` instead —
   * it makes the intent obvious at the call site. `setRestrictions([])` is
   * equivalent but semantically ambiguous: future maintainers shouldn't have
   * to scan the implementation to learn that an empty array = wide open.
   *
   * @param {Array} restrictions - Active restriction rows
   * @param {string} [requestId] - Unique ID for the current request (state-leak detection)
   */
  ...
  /**
   * Explicitly disable all table/field restrictions for the current process
   * scope. Use at entry points where field-level security isn't the concern
   * (writeback endpoints, admin scripts, CRM-backed audit logging).
   *
   * Preferred over `setRestrictions([])` — keeps the "this bypasses security
   * masking" decision visible in every call site.
   *
   * @param {string} [requestId] - Context label for state-leak detection logs
   */
  ...
  static checkRestriction(tableName, selectFields, expandParam, requestId) {
    // Fail-closed: if restrictions were never initialized, block all queries
    ...
    if (requestId && _restrictionRequestId && requestId !== _restrictionRequestId) {
      console.warn(`[DynExp SECURITY] Restriction state mismatch: expected ${_restrictionRequestId}, got ${requestId}. Possible request interleaving.`);
    }
  ...
  static async createRecord(entitySet, data) {
    ...
    if (!resp.ok) {
      const errorBody = await resp.text();
      throw new Error(`Create ${entitySet} failed (${resp.status}): ${errorBody}`);
    }
   ...
   */
  static async updateRecord(entitySet, recordId, data, { ifMatch } = {}) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
    ...
    if (!resp.ok) {
      const errorBody = await resp.text();
      const err = new Error(`Update ${entitySet}(${recordId}) failed (${resp.status}): ${errorBody}`);
   ...
   */
  static async updateIfEmpty(entitySet, recordId, fieldName, value, { overwrite = false, extraSelect = ['modifiedon'] } = {}) {
    ...
    try {
      await this.updateRecord(
        ...
        recordId,
        { [fieldName]: value },
        existing?._etag ? { ifMatch: existing._etag } : undefined,
      );
      return isEmpty
   ...
   * @returns {Promise<void>}
   */
  static async deleteRecord(entitySet, recordId) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
    const url = `${baseUrl}/api/data/v9.2/${entitySet}(${recordId})`;
   ...
   * @param {object|string} [opts.rawOutput] - Full structured payload; stringified if object
   * @param {string} [opts.notes] - Failure messages / retry context
   * @returns {Promise<{ runId: string, runNum: string }>} IDs of the created row
   */
  static async logAiRun({ requestGuid, taskType, model, promptVersion, status, rawOutput, notes }) {
    ...
    const created = await this.createRecord('wmkf_ai_runs', payload);
    return {
  ...
  static async createEmailActivity({ subject, body, from, to, cc, regardingId, regardingType }) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
    ...
    const resp = await fetchWithTimeout(`${baseUrl}/api/data/v9.2/emails`, {
      method: 'POST',
      headers: {
        ...this.buildHeaders(token),
        Prefer: 'return=representation',
   ...
   */
  static async addEmailAttachment(emailId, { filename, contentType, content }) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
    ...
    const resp = await fetchWithTimeout(`${baseUrl}/api/data/v9.2/activitymimeattachments`, {
      method: 'POST',
      headers: this.buildHeaders(token),
   ...
   */
  static async sendEmail(emailId) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;

    const resp = await fetchWithTimeout(`${baseUrl}/api/data/v9.2/emails(${emailId})/Microsoft.Dynamics.CRM.SendEmail`, {
      method: 'POST',
      headers: this.buildHeaders(token),
      body: JSON.stringify({
        IssueSend: true,
   ...
   */
  static async createAndSendEmail({ subject, body, from, to, cc, regardingId, regardingType, attachments = [] }) {
    // Step 1: Create the email activity
    const emailId = await this.createEmailActivity({ subject, body, from, to, cc, regardingId, regardingType });
