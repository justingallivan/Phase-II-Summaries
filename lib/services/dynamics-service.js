...
export class DynamicsService {
   ...
   */
  static setRestrictions(restrictions) {
   ...
   */
  static checkRestriction(tableName, selectFields) {
  ...
  static async createEmailActivity({ subject, body, from, to, cc, regardingId, regardingType }) {
    ...
    parties.push({ participationtypemask: 1, addressused: from });
  ...
  static async sendEmail(emailId) {
    ...
    const resp = await fetchWithTimeout(`${baseUrl}/api/data/v9.2/SendEmail`, {
      method: 'POST',
      headers: this.buildHeaders(token),
      body: JSON.stringify({
        EmailId: emailId,
