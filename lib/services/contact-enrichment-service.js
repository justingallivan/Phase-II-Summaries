...
class ContactEnrichmentService {
  ...
  /**
   * Save enrichment results to database (only if researcher already exists)
   * Researchers are only created when explicitly saved by the user.
   */
  static async saveToDatabase(candidate, enrichment) {
    try {
      // Only update if researcher already exists in database
      const existing = await DatabaseService.findResearcher(candidate.name);
      if (!existing) {
        // Researcher hasn't been saved by user yet - skip database update
        return;
      }

      // Update existing researcher with enrichment data
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: `Find email and faculty page for ${cleanName}${institution ? ` at ${institution}` : ''}. Return JSON: {"email":"...","facultyPageUrl":"...","website":"..."}`,
          },
        ],
      }),
    });
