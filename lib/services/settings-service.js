 ...
 *
 * Routes to Dataverse by default. The Postgres branches are dead code —
 * the underlying `system_settings` table was dropped 2026-05-12 (Wave 1
 * closeout). The branches remain only because removing them is a larger
 * refactor; if `WAVE1_BACKEND_SETTINGS=postgres` is set explicitly, the
 * Postgres path will fail loudly with "relation does not exist", which
 * is the desired behavior (an explicit opt-in to a dead path).
 *
...
function useDataverse() {
  // Default to Dataverse; only an explicit 'postgres' opts out (and will fail
  // loudly since the Postgres table no longer exists post Wave 1 closeout).
  return process.env.WAVE1_BACKEND_SETTINGS !== 'postgres';
}
