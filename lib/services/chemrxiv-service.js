  ...
  static async search(query, maxResults = 50, options = {}) {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        skip: '0',
        sort: 'RELEVANT_DESC' // Sort by relevance (descending = most relevant first)
      });
      ...
      // Log results summary
      console.log(`[ChemRxiv] Query "${query.substring(0, 40)}..." → ${data.totalCount || 0} total, ${articles.length} returned`);
      if (articles.length > 0) {
        console.log(`[ChemRxiv] Sample authors:`, articles.slice(0, 3).map(a => a.correspondingAuthor || a.authors?.[0]).join(', '));
      }
