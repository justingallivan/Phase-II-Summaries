...
import { buildBoundedTextPayload } from '../utils/ai-payload-boundary.js';
...
export async function executePrompt({
  ...
  try {
    ...
    const { variables, aiPayloadBoundaries } = applyVariableBoundaries(
      variableDecls,
      rawVariables,
...
async function persistOutputs(outputs, parsed, requestId, etag, actingUserSystemId = null) {
  ...
  // Validate target kinds + collect akoya_request writes. Non-akoya targets
  // ('none' = pass-through-only) get a `skipped` result row so callers can
  // see them. Anything else is a programmer error and throws.
  const akoyaWrites = [];
  for (const out of outputs) {
 ...
 * variable that opts in via `dataClass` + `maxChars`, apply
 * `buildBoundedTextPayload` and collect metadata.
 *
 * Variables without both fields pass through unchanged. Source string for the
 * helper is `executor.<promptName>.<variableName>` so audit logs and tests can
 * distinguish Executor-driven boundaries from route-driven ones.
 *
 ...
 *
 * @returns {{variables: Object, aiPayloadBoundaries: Array}}
 */
function applyVariableBoundaries(decls, resolved, promptName) {
  ...
  return { variables, aiPayloadBoundaries };
}
