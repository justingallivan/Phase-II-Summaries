...
class SerpContactService {
  ...
  static async findScholarProfile(candidate, apiKey = null) {
    ...
    // Use just the name for scholar profiles search (institution can cause issues)
    const query = cleanName;

    console.log('Google Scholar search:', query);

    try {
      // Use google_scholar_author_search which is more reliable
      const params = new URLSearchParams({
      ...
      if (!response.ok) {
        // Try alternate approach using regular Google search for Scholar profile
        const errorText = await response.text();
        console.error('Scholar API error:', response.status, errorText.substring(0, 200));

        // Fall back to Google search for Scholar profile
  ...
  /**
   * Fallback: Find Google Scholar profile using regular Google search
   * Used when the Scholar profiles API returns an error
   */
