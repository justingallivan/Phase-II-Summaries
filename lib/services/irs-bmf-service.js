 ...
 *   (cross-region overlaps). Use for first-load verification or when
 *   the cron should refuse silent data-quality degradation. Live table
 *   stays untouched on strict failure; the error message carries the
 *   per-region anomaly counts.
...
async function refresh({ dryRun = false, strict = false } = {}) {
  ...
  try {
    ...
    await client.query(`
      ...
      WHERE ctid IN (
        SELECT ctid FROM (
          SELECT ctid, ROW_NUMBER() OVER (PARTITION BY ein ORDER BY ctid) AS rn
