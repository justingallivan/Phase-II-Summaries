...
async function refresh({ dryRun = false } = {}) {
  ...
  try {
    ...
    for (const region of Object.keys(REGION_URLS)) {
      ...
      const rowCount = await streamRegionIntoStaging(client, region, csvPath, refreshDate);
      stats.perRegion[region] = { rows: rowCount, csvSizeBytes: size };
