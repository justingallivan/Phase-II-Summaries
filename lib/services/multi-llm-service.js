...
export class MultiLLMService {
   ...
   * @param {number} options.maxTokens - Max response tokens (default 8192)
  ...
  static async call(provider, prompt, options = {}) {
    ...
    for (let attempt = 0; attempt <= RETRY_CONFIG.MAX_RETRIES; attempt++) {
      try {
        ...
        switch (provider) {
        case 'claude':
          result = await this._callClaude(apiKey, prompt, systemPrompt, model, maxTokens, temperature);
          break;
        case 'openai':
          result = await this._callOpenAI(apiKey, prompt, systemPrompt, model, maxTokens, temperature);
          break;
        case 'gemini':
          result = await this._callGemini(apiKey, prompt, systemPrompt, model, maxTokens, temperature);
          break;
        case 'perplexity':
          result = await this._callPerplexity(apiKey, prompt, systemPrompt, model, maxTokens, temperature);
          break;
        default:
