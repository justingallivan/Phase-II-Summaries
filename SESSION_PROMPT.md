...
### G. Normalization (the named follow-up to the fact-consistency gate — NEW)
The `check:fact-consistency` gate is the bounded scalar-drift backstop; the structural follow-up is **reducing avoidable restatements to pointers**. Per Codex's plan (in the `0c0a442` commit message body and audit doc Phase 5): create `docs/CANONICAL_COUNTS.md` as a generated source for code-derived scalars; replace non-operational literal counts with `See docs/CANONICAL_COUNTS.md` pointers; add a CI lint that validates `fact=<fact-id>` targets exist in the registry (canonical-pointer-rot detection). Not started; not blocking anything; do not over-build.
