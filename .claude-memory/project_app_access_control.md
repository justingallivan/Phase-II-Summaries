...
- **`shared/config/appRegistry.js`** — single source of truth for all 17 app definitions (keys, names, icons, categories, descriptions); used by Layout nav, home page, admin dashboard, and access control
...
- **API-level enforcement active** — `requireAppAccess(req, res, ...appKeys)` on ~52 app endpoints (verified 2026-05-19 via `grep -rln "requireAppAccess" pages/api/`; was ~48 at S154 2026-05-14)
