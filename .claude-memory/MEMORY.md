...
- [Reconcile docs, don't append-patch](feedback_reconcile_dont_append_docs.md) — recurring failure (S157/S158/S166, ≥3× in one session): fixing a fact in one place, stale restatements rot elsewhere. Edit the whole doc to one consistent state; grep all restatements; top-says-X/tail-says-not-X is a P0. **S166: `check:fact-consistency` gate now mechanically enforces this for code-derived scalars; emit no "DONE" for fact-level doc/memory edits until the fan-in (gate/grep/Codex) is green.**
- [Two memory stores — propagation root cause](project_memory_two_stores_propagation.md) — `.claude-memory/` (git-tracked, snake_case, propagates, `/stop` commits it) vs `~/.claude/.../memory/` (kebab harness store, per-machine, does NOT propagate). Active writes silently moved to the harness store ~S161–164 = "memories stopped propagating"; iCloud can't fix it. "Phantom memory" belief was a store-divergence artifact (the named ones DO exist here). Reconverge on `.claude-memory/`; dual-write meanwhile; repo OUT of iCloud. Open Justin decision.
...
- [Relay Codex output verbatim](feedback_codex_relay_verbatim.md) — every Codex round-trip: paste stdout exactly, as the ENTIRE response. No commentary before or after (S155 tightening supersedes the old "after is OK"); raise decisions in a separate turn. Acting on findings afterward is fine.
...
- [Interim grant report auto-evaluation](project_interim_report_automation.md) — backend job to evaluate yearly interim reports + write to Dynamics. Unblocked; field/prompt/process design still needed.
...
- [Reviewer identity fragmentation](project_reviewer_identity_fragmentation.md) — S158 *sample-based flag* (5/87 rows + architecture, Postgres not join-tested): a reviewer appears to span ≥4 disjoint stores (dirty auto-created `contact` + GOapply object + honorarium `akoya_request` + Postgres `researchers`) with no shared key; forward constraint on Reviewer Manager → Dataverse, not an exhaustive census
...
- [Institution contact-role triad](project_institution_foundation_liaison.md) — WMKF domain fact (user-attested S159): `akoya_primarycontactid` = foundation liaison/steward (NOT PI), `wmkf_projectleader` = the PI/scientific lead (program-conditional), `wmkf_researchleader` = institutional research officer (NOT PI); disclose accordingly anywhere they surface
