...
**How to apply:**
- At session start, manually run `npm run check:atlas && npm run check:api-routes`. If any are red, report them as the FIRST item in the session summary, before recapping the prior session. The /start skill does NOT run these automatically — this is a manual step.
