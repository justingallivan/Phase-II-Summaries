...
**Why:** On 2026-05-03 the audit found that "drop dormant Postgres reviewer tables" was sitting in the Session 126 pivot list as a green-lit option. The tables were not dormant — they had 20+ live UPDATE sites in `database-service.js` and `pages/api/reviewer-finder/researchers.js`, plus active reads in `extract-summary.js`, `generate-emails.js`, `grant-cycles.js`, and `my-proposals.js`. Acting on the carryover would have broken the live Reviewer Finder app's browse/email/grant-cycle flows.
