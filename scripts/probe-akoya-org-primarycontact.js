#!/usr/bin/env node
/**
 * akoya_primarycontactid spot-check for one applicant org (READ-ONLY) —
 * generalized from the USC spot-check; reusable for recognition-pass
 * cross-checks. Tests whether "Primary Contact" is the PI or the
 * institution's grant/administration contact-of-record, per era, plus
 * in-practice null rate.
 *
 * Usage: node scripts/probe-akoya-org-primarycontact.js "<name LIKE>" "<aka LIKE>" <slug>
 *   defaults: "University of Southern California" "USC" usc
 * Metrics/sample queries key on the NAME pattern only (the aka clause is
 * discovery-only — loose substrings false-match, an honest caveat).
...
const NAME_LIKE = process.argv[2] || 'University of Southern California';
const AKA_LIKE = process.argv[3] || 'USC';
const SLUG = process.argv[4] || 'usc';
...
(async () => {
  ...
  const cnt = async (extra) => {
    const fx = `<fetch aggregate="true"><entity name="akoya_request">` +
      ...
      `<filter>${nameCond}</filter></link-entity></entity></fetch>`;
  ...
  console.log('══ akoya_primarycontactid fill ══');
  ...
  const fx = `<fetch top="25"><entity name="akoya_request">` +
    ...
    `<link-entity name="account" from="accountid" to="akoya_applicantid" link-type="inner" alias="a">` +
    `<attribute name="name" alias="appl"/><filter>${nameCond}</filter></link-entity></entity></fetch>`;
  const s = await get(token, `/akoya_requests?fetchXml=${encodeURIComponent(fx)}`);
  console.log('══ requests — newest by decision date (Primary Contact in context) ══');
  ...
  console.log(`\nDone (read-only ${SLUG} primary-contact spot-check).`);
